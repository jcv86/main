...
const CHILE_REGIONS=[['Arica y Parinacota',['arica','parinacota']],['Tarapacá',['iquique','tarapaca']],['Antofagasta',['antofagasta','calama']],['Atacama',['copiapo','atacama']],['Coquimbo',['la serena','coquimbo']],['Valparaíso',['valparaiso','vina del mar','quilpue']],['Metropolitana',['santiago','metropolitana','las condes','providencia','vitacura','maipu','pudahuel']],['O’Higgins',['rancagua','ohiggins']],['Maule',['talca','curico','maule']],['Ñuble',['chillan','nuble']],['Biobío',['concepcion','talcahuano','biobio']],['La Araucanía',['temuco','araucania']],['Los Ríos',['valdivia','los rios']],['Los Lagos',['puerto montt','osorno','los lagos']],['Aysén',['coyhaique','aysen']],['Magallanes',['punta arenas','magallanes']]] as const
export function inferChileRegion(location:string|null,title?:string){const raw=[location,title].filter(Boolean).join(' ');if(!raw)return null;const n=norm(raw);for(const [region,terms] of CHILE_REGIONS){if(terms.some(t=>n.includes(norm(t))))return region}return null}
export function regionCatalogFromJobs(jobs:{location:string|null}[]){const counts=new Map<string,number>();for(const job of jobs){const region=inferChileRegion(job.location);if(region)counts.set(region,(counts.get(region)||0)+1)}return [...counts].map(([label,count])=>({label,count})).sort((a,b)=>b.count-a.count||a.label.localeCompare(b.label))}
