...
export function inferChileRegion(location:string|null){if(!location)return null;const n=norm(location);for(const [region,terms] of CHILE_REGIONS){if(terms.some(t=>n.includes(norm(t))))return region}return null}
