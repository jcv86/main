...
| DTC-QA-01 | Existe una puerta reproducible de TypeScript, contratos críticos y build | `npm run quality:gate`; candidato #166 `e888ac1` con 10/10 workflows PASS y Vercel READY; PR apilado #167 también verde en contrato OPS, TypeScript, build, Design, Pilot y continuidad | QA | Ninguna | verified | sí |
| DTC-QA-02 | Invitado nuevo y usuario recurrente completan un recorrido autenticado real | Laboratorio: Auth/JWT/PostgREST reales locales, guardar/recargar/retomar, desktop, 390×844, print, aislamiento y A1→A4 PASS. Falta claim + OAuth real en Preview protegido y cierre de sesión extremo a extremo allí | QA | Preview OAuth real | in_progress | sí |
