...
CREATE TABLE IF NOT EXISTS user_activities (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    metadata JSONB,
    UNIQUE(user_email, activity_description)
);
...
CREATE TABLE IF NOT EXISTS interview_simulations (
    ...
    metadata JSONB,
    UNIQUE(user_email, simulation_type, completed_at)
);
...
CREATE TRIGGER update_generated_cvs_updated_at BEFORE UPDATE ON generated_cvs FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_knowledge_base_updated_at BEFORE UPDATE ON knowledge_base_documents FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
