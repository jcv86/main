-- ═══════════════════════════════════════════════════════════════════════════
-- REPORTE COMPLETO DEL ESTADO DE LA BIBLIOTECA - CORREGIDO
-- ═══════════════════════════════════════════════════════════════════════════

-- 1. Resumen general por categoría de longitud
WITH categorias_longitud AS (
    SELECT 
        id,
        title,
        author,
...
SELECT 
    ...
    ROUND(AVG(longitud)::numeric, 0) as promedio_caracteres,
    ROUND(AVG(longitud / 200.0)::numeric, 1) as promedio_minutos_lectura
FROM categorias_longitud
GROUP BY categoria_longitud
ORDER BY 
    CASE categoria_longitud
        WHEN '🟢 Excelente (50k+)' THEN 1
        WHEN '🟡 Muy Bueno (35-50k)' THEN 2
        WHEN '🟠 Bueno (20-35k)' THEN 3
        WHEN '🔴 Regular (10-20k)' THEN 4
        ELSE 5
    END;
...
SELECT 
    ...
    ROUND(SUM(LENGTH(content) / 200.0)::numeric / 60, 1) as total_horas_lectura,
