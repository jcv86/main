...
const progressSidebar = source('components/a2-progress-sidebar.tsx')

assert.ok(progressRoute.includes("export const dynamic = 'force-dynamic'"))
assert.ok(progressRoute.includes('export const revalidate = 0'))
assert.ok(progressRoute.includes("'Cache-Control': 'private, no-store"))
assert.ok(progressRoute.includes("Vary: 'Cookie'"))
assert.ok(progressSidebar.includes("cache: 'no-store'"))
...
assert.ok(dayTemplate.includes('const result = await completeA2Day('))
assert.ok(dayTemplate.includes('dayNumber,'))
