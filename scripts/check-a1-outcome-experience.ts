...
assert.ok(route.includes("error: 'A1 response evidence is required'"), 'server must require A1 response evidence')
assert.ok(route.includes("requiredResponseKeys = ['target', 'value', 'evidence', 'gap']"), 'server must enforce the four A1 evidence prompts')
