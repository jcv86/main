...
-- Verificar que todas las tablas tienen la columna user_email donde corresponde
SELECT 
    table_name,
    column_name,
    data_type
...
WHERE table_schema = 'public' 
    AND table_name IN ('test_results', 'user_activities', 'interview_simulations', 'generated_cvs', 'coach_conversations', 'user_achievements', 'user_skills')
...
-- Mostrar resumen completo del perfil creado
SELECT 
    'PERFIL CREADO EXITOSAMENTE' as status,
    name,
    email,
    profile_completion_percentage as "% Completado",
    total_xp as "XP Total",
    current_level as "Nivel",
    tests_completed as "Tests",
    cv_generated as "CV",
    interview_simulations as "Entrevistas"
FROM user_profiles 
WHERE email = 'travis@nuanu.com';

-- Mostrar tests completados
SELECT 
    'TESTS COMPLETADOS' as status,
    test_name as "Test",
    score as "Score",
    completed_at as "Fecha"
FROM test_results 
WHERE user_email = 'travis@nuanu.com'
ORDER BY completed_at DESC;

-- Mostrar actividades recientes
SELECT 
    'ACTIVIDADES RECIENTES' as status,
    activity_description as "Actividad",
    xp_earned as "XP",
    created_at as "Fecha"
