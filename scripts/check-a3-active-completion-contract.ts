...
assert.ok(!completionRoute.includes('.insert('))
assert.ok(!completionRoute.includes('.upsert('))
...
assert.ok(progressRoute.includes('totalModules:'))
assert.ok(progressRoute.includes('moduleStates'))
...
assert.ok(cvStudio.includes('useA3DraftStorage({'))
