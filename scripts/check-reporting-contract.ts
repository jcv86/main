...
const a1Model = source('lib/reports/a1-professional-report.ts')
...
assert.ok(integral.includes('loadA1Report'))
assert.ok(integral.includes('Abrir informe profesional A1'))
assert.ok(data.includes(".from('a1_cerebral_assessment')"))
assert.ok(data.includes(".from('canon_conozcamonos_1_responses')"))
assert.ok(data.includes(".from('canon_conozcamonos_2_responses')"))
assert.ok(a1Model.includes('discNetScoreToIntensity'))
assert.ok(!a1Model.includes('Math.abs'))
