...
write('middleware.ts', readFileSync(join(repo, 'middleware.ts'), 'utf8'))
write('app/globals.css', readFileSync(join(repo, 'app/globals.css'), 'utf8'))
write('app/layout.tsx', `import './globals.css'
import {LabSignOut} from '@/components/lab-sign-out'
export default function Layout({children}:{children:React.ReactNode}) { return <html lang="es" className="dark"><body style={{'--font-montserrat':'Arial','--font-lora':'Georgia'} as React.CSSProperties}><header data-lab-chrome className="border-b p-4 text-sm">LABORATORIO SINTÉTICO · componentes reales, no producción<nav className="mt-2 flex flex-wrap gap-4"><a href="/despega/a1-report">A1</a><a href="/despega/career-identity">Identidad profesional</a><a href="/despega/settings">Configuración</a><a href="/auth/signin">Acceso de laboratorio</a><LabSignOut /></nav></header><main id="main-content"><div className="mx-auto max-w-7xl p-4">{children}</div></main></body></html> }
`)
write('components/lab-sign-out.tsx', `'use client'
import {useRouter} from 'next/navigation'
export function LabSignOut(){const router=useRouter();return <button type="button" onClick={async()=>{await fetch('/api/auth/signout',{method:'POST'});router.replace('/auth/signin');router.refresh()}}>Cerrar sesión de laboratorio</button>}
`)
...
export default function Login(){const [email,E]=useState(''),[password,P]=useState(''),[error,X]=useState('');const router=useRouter(),params=useSearchParams();return <form className="mx-auto max-w-md space-y-4 py-8" onSubmit={async e=>{e.preventDefault();const result=await createClient().auth.signInWithPassword({email,password});if(result.error){X('No se pudo iniciar la sesión');return}router.replace(normalizeNextPath(params.get('next')));router.refresh()}}><h1 className="text-2xl">Acceso sintético de laboratorio</h1><label className="block">Correo de prueba<input type="email" value={email} onChange={e=>E(e.target.value)} className="block border p-2 text-black"/></label><label className="block">Contraseña de prueba<input type="password" value={password} onChange={e=>P(e.target.value)} className="block border p-2 text-black"/></label><button type="submit" className="border p-3">Entrar al laboratorio</button><p role="status">{error}</p></form>}
`)
...
write('app/api/auth/signout/route.ts', readFileSync(join(repo, 'app/api/auth/signout/route.ts'), 'utf8'))
writeFileSync(join(root, 'evidence/scope.json'), JSON.stringify({ commit: process.env.A1_SOURCE_COMMIT, environment: 'disposable-local-supabase-and-next', genuine: ['Supabase Auth', 'JWT', 'PostgREST', 'PostgreSQL', 'actual production middleware and sign-out route', 'actual A1 save, clarification and preferences route exports', 'actual report, identity and settings components', 'Chromium at 1440 and 390x844'], substituted: ['minimal laboratory page shell and credential form in place of external OAuth UI', 'C1/C2 capture seeded as synthetic fixtures', 'pilot admission function is a lab allowlist fixture', 'system fallback font, not production font loading'], notVerified: ['Google or LinkedIn provider exchange', 'Vercel preview OAuth allowlist', 'full production onboarding', 'A2-A4 integral route', 'psychometric validity'] }, null, 2))
