...
try{
 ...
 await step('unauthenticated_route_and_save',async()=>{await page.goto(base+'/despega/a1-report');await page.waitForURL('**/login');const r=await browserSave(page,fixtures.ordinary);assert.equal(r.status,401)})
 ...
 await step('session_cookie_removed_is_rejected',async()=>{await page.goto(base+'/despega/a1-report');await visibleText(page,'Más allá de una combinación de letras');await c.clearCookies();const r=await page.evaluate(async()=>{const response=await fetch('/api/a1/clarifications',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({})});return response.status});assert.equal(r,401);await page.reload();await page.waitForURL('**/login')})
