...
try{
 ...
 await step('preferences_real_http_save_reload_and_restore',async()=>{
  let loaded=page.waitForResponse(r=>new URL(r.url()).pathname==='/api/preferences'&&r.request().method()==='GET')
  await page.goto(base+'/despega/settings');await loaded;await visibleText(page,'Preferencias de perfil')
  ...
  loaded=page.waitForResponse(r=>new URL(r.url()).pathname==='/api/preferences'&&r.request().method()==='GET')
  await page.reload();await loaded;assert.equal(await weekly.isChecked(),false)
  await weekly.check();wait=page.waitForResponse(r=>new URL(r.url()).pathname==='/api/preferences'&&r.request().method()==='POST')
  await page.getByRole('button',{name:'Guardar Cambios'}).click();assert.equal((await wait).status(),200)
  loaded=page.waitForResponse(r=>new URL(r.url()).pathname==='/api/preferences'&&r.request().method()==='GET')
  await page.reload();await loaded;assert.equal(await weekly.isChecked(),true)
 })
