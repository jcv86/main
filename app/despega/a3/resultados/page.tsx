...
export default async function A3ResultadosPage() {
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-10 text-white"><div className="mx-auto max-w-6xl space-y-8">
    ...
    {report.modules.length === 0 ? <Card className="border-amber-500/30 bg-amber-500/10"><CardContent className="p-6"><p className="font-semibold">Todavía no hay sesiones completadas.</p><p className="mt-2 text-sm text-amber-50/80">Completa un módulo de Entrenamiento para construir este reporte.</p></CardContent></Card> : <section className="grid gap-4 md:grid-cols-2">{report.modules.map((session) => <Card key={String(session.module_id)} className="border-slate-800 bg-slate-900/80"><CardHeader><CardTitle className="flex items-center justify-between gap-3 text-lg"><span>Módulo {session.module_number ?? '—'}</span><span className="text-purple-300">{session.score == null ? 'Completado' : `${session.score}/100`}</span></CardTitle></CardHeader><CardContent><p className="break-words text-sm text-slate-400">{String(session.module_id)}</p>{session.feedback && <p className="mt-3 text-sm text-slate-300">{session.feedback}</p>}</CardContent></Card>)}</section>}
