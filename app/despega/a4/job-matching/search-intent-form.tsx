...
export function SearchIntentForm({seedRole,onSaved}:{seedRole?:string|null;onSaved?:()=>void}) {
 ...
 <CardContent><form onSubmit={submit} className="space-y-5"><div className="grid gap-4 lg:grid-cols-3">
  ...
  <div className="space-y-2"><label className="text-sm font-medium">Regiones</label><details className="group relative"><summary className="flex h-11 cursor-pointer list-none items-center rounded-xl border border-input bg-background px-3 text-sm"><MapPin className="mr-2 h-4 w-4 text-muted-foreground"/><span className="truncate">{regions.length?regions.join(', '):'Todas las regiones'}</span></summary><div className="absolute z-30 mt-2 max-h-72 w-full overflow-auto rounded-xl border border-border bg-popover p-2 shadow-xl">{REGIONS.map(r=><label key={r} className="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-muted"><input type="checkbox" checked={regions.includes(r)} onChange={()=>toggle(r,setRegions)}/>{r}</label>)}</div></details><Chips values={regions} onRemove={v=>setRegions(x=>x.filter(i=>i!==v))}/></div>
  <div className="space-y-2"><label className="text-sm font-medium">Modalidad</label><details className="group relative"><summary className="flex h-11 cursor-pointer list-none items-center rounded-xl border border-input bg-background px-3 text-sm"><Briefcase className="mr-2 h-4 w-4 text-muted-foreground"/><span className="truncate">{modes.length?MODES.filter(([v])=>modes.includes(v)).map(([,l])=>l).join(', '):'Cualquier modalidad'}</span></summary><div className="absolute z-30 mt-2 w-full rounded-xl border border-border bg-popover p-2 shadow-xl"><label className="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-muted"><input type="checkbox" checked={modes.length===0} onChange={()=>setModes([])}/>Cualquier modalidad</label>{MODES.map(([v,l])=><label key={v} className="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-muted"><input type="checkbox" checked={modes.includes(v)} onChange={()=>toggle(v,setModes)}/>{l}</label>)}</div></details><Chips values={MODES.filter(([v])=>modes.includes(v)).map(([,l])=>l)} onRemove={label=>{const found=MODES.find(([,l])=>l===label);if(found)setModes(x=>x.filter(i=>i!==found[0]))}}/></div>
 </div>
