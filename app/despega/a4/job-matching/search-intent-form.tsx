...
export function SearchIntentForm({seedRole,onSaved}:{seedRole?:string|null;onSaved?:()=>void}) {
 ...
 return <Card className="overflow-hidden border-border bg-card shadow-sm"><CardHeader><p className="text-sm font-semibold uppercase tracking-[0.16em] text-[hsl(var(--dtc-indigo-300))]">Tu búsqueda laboral</p><CardTitle className="text-2xl">¿Qué tipo de oportunidad quieres encontrar ahora?</CardTitle><p className="max-w-3xl text-sm text-muted-foreground">Ya conocemos parte de tu recorrido. Confirma o ajusta hacia dónde quieres orientar la búsqueda. Tú decides el objetivo; DTC sólo ayuda a traducirlo a las distintas fuentes.</p></CardHeader><CardContent><form onSubmit={submit} className="space-y-7">
  <div><label className="text-sm font-semibold text-foreground">Cargos o direcciones que te interesan</label><div className="mt-3 flex flex-wrap gap-2">{roles.map(r=><button type="button" key={r} onClick={()=>setRoles(v=>v.filter(x=>x!==r))} className="rounded-full border border-border bg-muted px-3 py-1.5 text-sm text-foreground">{r} <span aria-hidden>×</span></button>)}</div><div className="mt-3 flex gap-2"><input value={role} onChange={e=>setRole(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();addRole()}}} placeholder="Ej.: Gerente de Riesgo" className="h-10 flex-1 rounded-md border border-input bg-background px-3 text-sm"/><Button type="button" variant="outline" onClick={addRole}>Agregar</Button></div><p className="mt-2 text-xs text-slate-500">Puedes agregar varios. Toca uno para quitarlo.</p></div>
  <fieldset><legend className="text-sm font-semibold text-slate-900">¿Qué tan amplia quieres la búsqueda?</legend><div className="mt-3 grid gap-3 md:grid-cols-3">{BREADTHS.map(item=><label key={item.value} className={`cursor-pointer rounded-xl border p-4 ${breadth===item.value?'border-[hsl(var(--dtc-indigo-300))] bg-muted/60 ring-1 ring-[hsl(var(--dtc-indigo-300))]':'border-border bg-card'}`}><input className="sr-only" type="radio" name="breadth" value={item.value} checked={breadth===item.value} onChange={()=>setBreadth(item.value)}/><span className="font-semibold text-slate-900">{item.title}</span><span className="mt-1 block text-xs leading-relaxed text-slate-600">{item.copy}</span></label>)}</div></fieldset>
  ...
  {message&&<p role="status" className="rounded-xl border border-border bg-muted/40 p-3 text-sm text-slate-700">{message}</p>}
  <Button disabled={saving} className="bg-[hsl(var(--dtc-indigo-900))] text-white hover:opacity-90">{saving?'Guardando…':'Buscar oportunidades para mí'}</Button>
