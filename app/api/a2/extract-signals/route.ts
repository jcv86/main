...
// Extract signals from jobs using keyword analysis
function extractSignalsFromJobs(marketSignals: MarketSignal[]) {
  ...
  for (const signal of marketSignals) {
    ...
    // Extract potential blockers/fears as high importance
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      market_analysis: `Analyzed ${marketSignals.length} job postings and extracted ${savedSignals.length} key market signals.`,
    })
  } catch (error) {
