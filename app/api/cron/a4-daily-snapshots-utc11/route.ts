...
/**
 * Cron: A4 Daily Snapshots (UTC+11)
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (usersError) {
      console.error('[Cron A4 UTC11] Error fetching users:', usersError)
      return NextResponse.json(
    ...
    for (const userProfile of users || []) {
      ...
      if (!snapshotError) {
        ...
        console.warn(`[Cron A4 UTC11] Failed to create snapshot for user ${userProfile.user_id}:`, snapshotError)
      }
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: `A4 Daily Snapshots (UTC+11) completed`,
    ...
    console.error('[Cron A4 UTC11] Unexpected error:', error)
    return NextResponse.json(
