...
function normalizeResponseTimings(value: unknown) {
  ...
  for (const item of value.slice(0, 28)) {
    ...
    const questionId = Number(item.questionId), responseTime = Number(item.responseTime)
    if (!Number.isInteger(questionId) || questionId < 1 || questionId > 28) continue
    if (!Number.isFinite(responseTime) || responseTime < 0 || responseTime > 3_600 || seen.has(questionId)) continue
    seen.add(questionId)
...
export async function POST(request: Request) {
  try {
    ...
    let payload: unknown
    try { payload = await request.json() } catch { return NextResponse.json({ error: 'La solicitud no contiene JSON válido.' }, { status: 400 }) }
    ...
    if (forbiddenField) return NextResponse.json({ error: `El campo ${forbiddenField} es calculado o resuelto por el servidor.`, code: 'client_owned_a1_field_rejected' }, { status: 400 })

    const scoring = validateAndScoreDiscResponses(body.responses)
    if (!scoring.valid || !scoring.value) return NextResponse.json({ error: 'Las respuestas del cuestionario no son válidas.', code: 'invalid_disc_responses', details: scoring.errors.slice(0, 8) }, { status: 422 })
