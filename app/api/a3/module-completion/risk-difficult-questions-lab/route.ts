...
export async function POST(request: NextRequest) {
  try {
    ...
    if (cvResult.error || decoderResult.error || simulationResult.error) {
      console.error('[v0] Difficult questions completion context error:', {
        cv: cvResult.error,
        decoder: decoderResult.error,
        simulation: simulationResult.error,
      })
      return NextResponse.json(
        { error: 'No pudimos verificar el CV, la oferta y la simulación anteriores.' },
        { status: 500 },
      )
    }

    const validation = validateDifficultQuestionsSubmission(
      module,
      body.responses,
      body.deliverable,
    ...
    const { data, error } = await (supabase as any).rpc(
      'complete_a3_module_atomic',
      {
        ...
        p_complete_route: false,
      },
    )

    if (error) {
      console.error('[v0] Difficult questions atomic completion error:', error)
      return NextResponse.json(
        {
    ...
    console.error('[v0] Difficult questions completion error:', error)
    return NextResponse.json(
      { error: 'No pudimos completar el laboratorio.' },
      { status: 500 },
    )
  }
}
