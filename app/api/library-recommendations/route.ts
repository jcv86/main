...
export async function POST(request: NextRequest) {
  try {
    ...
    // Try to use OpenAI if available, otherwise use curated recommendations
    ...
    try {
      ...
      if (process.env.OPENAI_API_KEY) {
        ...
        const prompt = `
        Eres un experto en desarrollo profesional para el mercado laboral chileno. 
        
        Perfil del usuario:
        - Nivel de lectura: ${userLevel}/5
        - Libros leídos: ${booksRead}
        - Tiempo total de lectura: ${readingTime} minutos
        
        Libros disponibles:
        1. "Hábitos Atómicos" por James Clear - Productividad, Intermedio
        2. "Trabajo Profundo" por Cal Newport - Productividad, Intermedio  
        3. "Los 7 Hábitos de la Gente Altamente Efectiva" por Stephen R. Covey - Liderazgo, Intermedio
        4. "De Buena a Grandiosa" por Jim Collins - Negocios, Avanzado
        5. "Conversaciones Cruciales" por Kerry Patterson - Habilidades Blandas, Intermedio
        
        Recomienda exactamente 3 libros en formato JSON:
        {
          "recommendations": [
            {
              "bookId": "1",
              "reason": "razón específica para este usuario",
              "priority": "high|medium|low",
              "expectedBenefit": "beneficio específico para el mercado chileno"
            }
          ]
        }
        `
        ...
        // Map AI response to full book objects
      ...
      console.log("Using curated recommendations (OpenAI not available)")
      recommendations = getCuratedRecommendations(userLevel, booksRead)
    }

    return NextResponse.json({
      success: true,
      recommendations,
      source: process.env.OPENAI_API_KEY ? "ai" : "curated",
    })
  } catch (error) {
