-- DTC Career Identity foundation
-- Additive migration: no existing A1-A4 tables are modified or removed.
...
create table if not exists public.career_skills (
  ...
  trend text not null default 'stable' check (trend in ('declining','stable','improving','unknown')),
...
create index if not exists career_evidence_skill_idx on public.career_evidence(skill_id) where skill_id is not null;
...
create trigger set_career_identities_updated_at
before update on public.career_identities
...
create policy "career_identities_owner_all" on public.career_identities
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "career_goals_owner_all" on public.career_goals
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "career_skills_owner_all" on public.career_skills
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "career_skill_edges_owner_all" on public.career_skill_edges
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "career_evidence_owner_all" on public.career_evidence
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "career_memories_owner_all" on public.career_memories
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "career_profile_snapshots_owner_all" on public.career_profile_snapshots
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

comment on table public.career_identities is 'Canonical longitudinal Career Identity; additive to existing A1-A4 storage.';
comment on table public.career_evidence is 'Explainable observations supporting profile and skill assertions.';
comment on table public.career_memories is 'Versionable long-term facts, preferences, patterns and context.';
