  ...
  return (
    <div className="space-y-4">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-[rgb(170,70,170)]">Day {dayNumber}</p>
              <p className="text-xs text-white/60">of 90</p>
            </div>
          </div>
        </CardHeader>
      </Card>
      ...
      <Card className="border-[rgb(170,70,170)]/30 bg-black">
        <CardHeader>
          <CardTitle className="text-white text-sm">Route Progress Overview</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-white/70">XP Earned</span>
              <span className="text-sm font-bold text-[rgb(170,70,170)]">
                {metrics.xpEarned} / 1,340
              </span>
            </div>
            <Progress value={Math.min(xpPercent, 100)} className="h-2" />
          </div>

          <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/10">
            <div>
              <p className="text-xs text-white/60">Avg Response Rate</p>
              <p className="text-lg font-bold text-cyan-400">{metrics.averageResponseRate.toFixed(1)}%</p>
            </div>
            <div>
              <p className="text-xs text-white/60">Avg Interview Rate</p>
              <p className="text-lg font-bold text-purple-400">{metrics.averageInterviewRate.toFixed(1)}%</p>
            </div>
          </div>
        </CardContent>
      </Card>
