...
import { getGamifiedTests, getUserTestCompletions, submitTestAnswers } from "@/lib/supabase/a4-queries"
...
export function GamifiedTests() {
  ...
  const loadData = async () => {
    ...
    try {
      const [testsData, completionsData] = await Promise.all([
        getGamifiedTests(10),
        getUserTestCompletions(session.user.id),
      ])
