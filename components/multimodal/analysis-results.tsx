...
export function AnalysisResults({ sessionId }: AnalysisResultsProps) {
  ...
  if (loading) {
    return (
      <Card className="border-ritual/20">
        <CardContent className="pt-6 flex flex-col items-center justify-center gap-4">
          <Loader2 className="w-8 h-8 animate-spin text-ritual" />
  ...
  return (
    <div className="space-y-6">
      {/* Overall Score - Ritual color */}
      <Card className="border-2 border-ritual/30 bg-gradient-to-r from-ritual/5 to-transparent">
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span>Tu Puntuación General</span>
            <Badge className="bg-ritual text-white font-bold">{analysis.overall_score}/100</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card className="border-ritual/20">
        <CardHeader>
          <CardTitle className="text-lg text-ritual">Análisis Visual</CardTitle>
          <CardDescription>Postura, contacto visual, gestos y expresiones</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-ritual/5 border border-ritual/10 rounded-lg p-4">
              <p className="text-sm text-muted-foreground mb-2">Postura</p>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-bold text-ritual">{analysis.visual_analysis.posture_quality}</span>
            ...
            <div className="bg-ritual/5 border border-ritual/10 rounded-lg p-4">
              ...
              <div className="flex items-end gap-2">
                <span className="text-3xl font-bold text-ritual">{analysis.visual_analysis.eye_contact}</span>
                <span className="text-muted-foreground">/100</span>
              </div>
      ...
      <Card className="border-ritual/20">
        <CardHeader>
          <CardTitle className="text-lg text-ritual">Análisis de Audio</CardTitle>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-ritual/5 border border-ritual/10 rounded-lg p-4">
              ...
              <div className="flex items-end gap-2">
                <span className="text-3xl font-bold text-ritual">
            ...
            <div className="bg-ritual/5 border border-ritual/10 rounded-lg p-4">
              ...
              <div className="flex items-end gap-2">
                <span className="text-3xl font-bold text-ritual">{analysis.audio_analysis.confidence_level}</span>
                <span className="text-muted-foreground">/100</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm border-t border-border/30 pt-4">
            <div>
              ...
              <p className="font-bold text-ritual">{analysis.audio_analysis.speech_pace} PPM</p>
            </div>
            <div>
              <p className="text-muted-foreground mb-1 font-medium">Palabras de Relleno</p>
              <p className="font-bold text-ritual">{analysis.audio_analysis.filler_words}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-ritual/20">
        <CardHeader>
          <CardTitle className="text-lg text-ritual">Coherencia (Visual + Audio)</CardTitle>
          <CardDescription>Alineación entre lenguaje verbal y corporal</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex justify-between items-center p-3 bg-ritual/5 border border-ritual/10 rounded-lg">
            <span className="text-sm font-bold">Alineación Visual-Audio</span>
            <Badge className="bg-ritual text-white font-bold">{analysis.coherence_analysis.visual_audio_alignment}%</Badge>
          </div>
      ...
      <Card className="border-2 border-ritual/30 bg-ritual/5">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-ritual">
          ...
          <ul className="space-y-2">
            {analysis.key_strengths.map((strength: string, i: number) => (
              <li key={i} className="flex gap-2 text-sm">
                <span className="text-ritual font-bold">✓</span>
                <span>{strength}</span>
              </li>
            ))}
          </ul>
      ...
      <Card className="border-2 border-ritual/30">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-ritual">
        ...
        <CardContent className="space-y-3">
          {analysis.personalized_recommendations.map((rec: string, i: number) => (
            <div key={i} className="flex gap-3 p-3 bg-ritual/5 border border-ritual/10 rounded-lg">
              <span className="font-bold text-ritual flex-shrink-0">{i + 1}.</span>
      ...
      <Card className="border-ritual/20">
        <CardHeader>
          <CardTitle className="text-lg text-ritual">Feedback Detallado</CardTitle>
        </CardHeader>
        <CardContent>
