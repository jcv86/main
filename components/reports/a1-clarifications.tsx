...
interface Props {
  ...
  initial: ClarificationAnswers | null
  stale: boolean
}
...
export function A1Clarifications({ revision, editRevision, questions, initial, stale }: Props) {
  ...
  const save = async (clear = false) => {
    ...
    try {
      const response = await fetch('/api/a1/clarifications', {
        method: 'PUT', credentials: 'include', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ revision, editRevision, answers: next }),
      })
      const payload = await response.json().catch(() => ({})) as { error?: string; success?: boolean }
      if (!response.ok || !payload.success) {
        if (response.status === 409) setConflict(true)
        throw new Error(payload.error || 'No pudimos guardar tus aclaraciones.')
      }
      setAnswers(next)
      setSaved(true)
    ...
    <section className="rounded-2xl border border-cyan-400/30 bg-cyan-500/5 p-5 sm:p-6 print:hidden" aria-labelledby="a1-clarifications-heading" data-report-actions>
      ...
      <form className="mt-6 space-y-6" onSubmit={(event) => { event.preventDefault(); void save() }}>
        {questions.map((question, index) => (
          <fieldset key={question.id} disabled={busy || conflict || saved} className="space-y-3">
            ...
            <div className="grid gap-2">
              {question.options.map((option) => (
                <label key={option.id} className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-700 p-3 text-sm leading-relaxed text-slate-200 focus-within:outline focus-within:outline-2 focus-within:outline-cyan-300">
                  <input type="radio" name={`clarification-${question.id}`} value={option.id} checked={answers.selections[question.id] === option.id}
                    onChange={() => { setAnswers((current) => ({ ...current, selections: { ...current.selections, [question.id]: option.id } })); changed() }} className="mt-1 shrink-0 accent-cyan-400" />
        ...
        <div className="flex flex-wrap gap-3">
          <Button type="submit" disabled={busy || conflict || saved}>{busy ? 'Guardando…' : saved ? 'Guardado' : 'Guardar mis matices'}</Button>
          {initial && <Button type="button" variant="outline" disabled={busy || conflict || saved} onClick={() => void save(true)}>Retirar mis aclaraciones</Button>}
          {conflict && <Button type="button" variant="outline" onClick={() => router.refresh()}>Recargar versión actual</Button>}
        </div>
      </form>
    </section>
  )
