...
export function ReportProvenance({ report }: { report: A1ProfessionalReport }) {
  const dates = [
    ['Evaluación A1', report.assessmentDate], ['Contexto inicial C1', report.provenance.c1CompletedAt],
    ['Contexto posterior C2', report.provenance.c2CompletedAt],
    ...(report.understanding.clarificationSavedAt ? [['Matices posteriores', report.understanding.clarificationSavedAt]] : []),
  ]
  return <footer className="border-t border-slate-700 pt-6 text-xs leading-relaxed text-slate-400 print:text-slate-700" data-report-provenance>
    <p className="font-semibold text-slate-200 print:text-slate-950">Documento personal de orientación · A1 “Despega Cerebral”</p>
    <dl className="mt-4 grid gap-3 sm:grid-cols-2">{dates.map(([label, value]) => <div key={label}><dt className="font-medium">{label}</dt><dd className="mt-1">{formatReportDate(value)}</dd></div>)}</dl>
